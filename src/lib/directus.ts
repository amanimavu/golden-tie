...
export async function getSubsidiaries(): Promise<Subsidiary[]> {
	try {
		if (!env.DIRECTUS_URL) return DUMMY_SUBSIDIARIES;

		const url = new URL("/items/subsidiaries", env.DIRECTUS_URL);
...
export async function getPrograms(): Promise<Program[]> {
	try {
		if (!env.DIRECTUS_URL) return DUMMY_PROGRAMS;

		const url = new URL("/items/programs", env.DIRECTUS_URL);
...
export function programImageUrl(program: Program): string | null {
    ...
    return new URL(`/assets/${program.image}`, env.DIRECTUS_URL).toString();
}
